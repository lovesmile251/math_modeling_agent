...
def evaluate_contest_readiness(
    ...
    *,
    ...
    results = [
        _score_case(item, time_budget_seconds=time_budget_seconds)
    ...
    summary = {
        ...
        "rubric": {
            "time_management": 10,
            "delivery_completeness": 15,
            "modeling_depth": 20,
            "result_validation": 20,
            "paper_competitiveness": 25,
            "reproducibility": 10,
        },
        "results": results,
    }
...
def format_contest_report(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "| 维度 | 分值 | 含义 |",
        "|---|---:|---|",
        "| 时间管理 | 10 | 是否能在单题预算内稳定完成，并留出人工检查余量 |",
        "| 交付完整性 | 15 | 代码、论文、质量报告、结果表、图形是否完整 |",
        "| 建模深度 | 20 | 已选核心模型是否实际产出，是否具备多模型支撑 |",
        "| 结果验证 | 20 | 是否有误差、敏感性、模型对比或证据追溯产物 |",
        "| 论文竞争力 | 25 | 论文质量分、表格、图形是否接近优秀论文结构密度 |",
...
def _score_case(item: dict[str, Any], *, time_budget_seconds: int) -> dict[str, Any]:
    ...
    time_score = 10 * _time_margin_score(elapsed, time_budget_seconds)
    delivery_score = 15 * _delivery_score(artifacts, table_count, figure_count)
    modeling_score = 20 * _modeling_score(produced_models, missing_models, empty_models)
    validation_score = 20 * _validation_score(produced_models, artifacts, table_names)
    paper_score = 25 * _paper_competitiveness_score(paper_quality, table_count, figure_count)
    ...
    blind_review_score = round(
        0.35 * paper_quality
        + 0.25 * min(100.0, 100.0 * _modeling_score(produced_models, missing_models, empty_models))
        + 0.20 * min(100.0, 100.0 * _validation_score(produced_models, artifacts, table_names))
        + 0.20 * min(100.0, 100.0 * _paper_artifact_density(table_count, figure_count)),
        2,
    )
    risks = _case_risks(
        elapsed=elapsed,
