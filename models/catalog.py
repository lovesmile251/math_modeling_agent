...
ALGORITHM_CATALOG: tuple[AlgorithmEntry, ...] = (
    ...
    _entry("优化类", "不确定性与鲁棒优化问题", ("鲁棒资源配置", "情景优化", "机会约束", "CVaR 优化"), ("随机规划", "分布鲁棒优化"), ("鲁棒", "不确定", "扰动", "情景", "风险", "稳健", "服务水平", "概率约束", "cvar", "尾部风险"), ("robust_optimization", "scenario_optimization", "chance_constrained_optimization", "cvar_optimization", "sensitivity_analysis")),
    ...
    _entry("机理建模", "传染病传播", ("SIR 模型", "SEIR 模型"), ("SIS 模型", "SIRD 模型"), ("传染病", "疫情", "传播", "sir", "seir"), ("sir_model", "seir_model")),
...
EXECUTABLE_MODEL_LABELS = {
    ...
    "robust_optimization": "鲁棒资源配置优化",
    "scenario_optimization": "多情景资源配置优化",
    "chance_constrained_optimization": "机会约束资源配置优化",
    "cvar_optimization": "CVaR 尾部风险资源配置优化",
...
MODEL_APPLICABILITY: dict[str, ApplicabilityRule] = {
    ...
    "cement_esp_optimization": ApplicabilityRule(("optimization", "simulation"), min_rows=20, min_numeric_cols=10, needs_time=True, role="primary", tier="innovation", validation="check outlet concentration target, power increment, and extrapolation level", not_recommended_when=("ESP voltage/residence-time columns are missing", "target outlet is far outside historical operation without engineering review"), innovation_extensions=("physics_informed_surrogate", "robust_setpoint_optimization")),
    ...
    "robust_optimization": ApplicabilityRule(("optimization", "simulation"), min_rows=2, min_numeric_cols=2, needs_objective_or_constraint=True, role="primary", tier="innovation", validation="check robust objective, capacity slack, and perturbation feasibility", not_recommended_when=("uncertainty is not decision-relevant", "deterministic optimum is required by the problem"), innovation_extensions=("scenario_programming", "chance_constrained_optimization")),
    "scenario_optimization": ApplicabilityRule(("optimization", "simulation"), min_rows=2, min_numeric_cols=2, needs_objective_or_constraint=True, role="primary", tier="innovation", validation="compare expected value, worst-case value, regret, and capacity feasibility across scenarios", not_recommended_when=("no meaningful scenario or uncertainty interpretation", "single deterministic decision is mandated"), innovation_extensions=("chance_constrained_optimization", "stochastic_programming")),
    "chance_constrained_optimization": ApplicabilityRule(("optimization", "simulation"), min_rows=2, min_numeric_cols=2, needs_objective_or_constraint=True, role="primary", tier="innovation", validation="check service level, feasibility probability, violation probability, and safe resource slack", not_recommended_when=("risk tolerance or service level is irrelevant", "uncertainty cannot be quantified"), innovation_extensions=("cvar_optimization", "distributionally_robust_optimization")),
    "cvar_optimization": ApplicabilityRule(("optimization", "simulation", "evaluation"), min_rows=2, min_numeric_cols=2, needs_objective_or_constraint=True, role="primary", tier="innovation", validation="check VaR, CVaR, tail scenario count, risk-adjusted score, and capacity feasibility", not_recommended_when=("tail risk is not decision-relevant", "loss distribution cannot be interpreted"), innovation_extensions=("distributionally_robust_optimization", "mean_cvar_portfolio")),
    ...
    "seir_model": ApplicabilityRule(("simulation",), min_rows=4, min_numeric_cols=4, role="primary", tier="improved", validation="check R0, incubation period, infectious period, and compartment residuals", not_recommended_when=("no exposed/latent compartment", "population is not conserved"), innovation_extensions=("time_varying_seir", "network_seir")),
    # Classification
